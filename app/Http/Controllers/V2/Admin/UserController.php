                ...
                foreach ($users as $user) {
                    try {
                        $row = [
                            ...
                            $user->expired_at ? date('Y-m-d H:i:s', $user->expired_at) : 'Không giới hạn',
                            $user->plan ? $user->plan->name : 'Không có gói',
    ...
    private function multiGenerate(Request $request)
    {
        ...
        if ($request->input('download_csv')) {
            ...
            $callback = function () use ($users, $request) {
                ...
                fputcsv($handle, ['Tài khoản', 'Mật khẩu', 'Ngày hết hạn', 'UUID', 'Ngày tạo', 'Địa chỉ đăng ký']);
                foreach ($users as $user) {
                    $user = $user->refresh();
                    $expireDate = $user['expired_at'] === NULL ? 'Không giới hạn' : date('Y-m-d H:i:s', $user['expired_at']);
        ...
        $data = collect($users)->map(function ($user) use ($request) {
            return [
                'email' => $user['email'],
                'password' => $request->input('password') ?? $user['email'],
                'expired_at' => $user['expired_at'] === NULL ? 'Không giới hạn' : date('Y-m-d H:i:s', $user['expired_at']),
                'uuid' => $user['uuid'],
        ...
        return response()->json([
            'code' => 0,
            'message' => 'Tạo hàng loạt thành công',
            'data' => $data,
        ]);
    }
